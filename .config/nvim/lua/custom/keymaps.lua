...
local function send_to_root()
  ...
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_is_loaded(buf)
      ...
      and vim.api.nvim_buf_get_name(buf):match("term:ROOT$") then
      target_buf = buf
      break
    end
  end
  ...
  if not target_buf then
    vim.cmd([[split | term bash -i -l -c '[ -n "$TMUX" ] && export DISPLAY=$(tmux show-env | sed -n 's/^DISPLAY=//p'); r']])
    target_buf = vim.api.nvim_get_current_buf()
    vim.api.nvim_buf_set_name(target_buf, "term:ROOT")
...
vim.keymap.set('v', '<leader>r', send_to_root, { desc = 'Send to term:ROOT' })
